var ChilLogo = (function(window) {
    ...
    var options = {
        containerID: "chil-logo",
        logoCanvasID: "chil-logo-canvas",
        shadowCanvasID: null,
        wordImageURL: "/assets/themes/dwtalk/img/chil.png",
        wordOffsetX: -25,
        wordOffsetY: 0,
        wordScale: 1,
    ...
    API.initialize = function(overrideOptions) {
        ...
        ChilLogo.util.extendObject(options, overrideOptions);
    ...
    API.applyImpulse = function(x, y, strength, radius) {
        function applyForceToShape(shape) {
            ...
            for (var i = 0, len = shape.points.length; i < len; i++) {
                ...
                var distance = ChilLogo.util.distanceBetween(target, p);
                if (distance < radius) {
    ...
    function enableDebugging() {
        ...
        var configureScript = "ChilLogo.initialize({\n";
        ...
        for (var i = 0; i < options.colors.length; i++) {
            ...
            configureScript += "\t\t\t [ new ChilLogo.util.Color( " + ca.r + ", " + ca.g + ", " + ca.r + " ), \t new ChilLogo.util.Color( " + ca.r + ", " + ca.g + ", " + ca.r + " ) ],\n"
        }
        configureScript += "\t\t ]\n";
        configureScript += "});";
    ...
    function updateColor() {
        ...
        if (targetColor != rgbToHex(shapeForegroundColorTarget.r, shapeForegroundColorTarget.g, shapeForegroundColorTarget.b)) {
            targetColor = rgbToHex(shapeForegroundColorTarget.r, shapeForegroundColorTarget.g, shapeForegroundColorTarget.b);
            $("#geek").animate({
                color: targetColor
            }, 0.015 * options.colorSpeedFactor * performance.factor * turboColorSpeedFactor * 300000);
            $("#geek2").animate({
                color: targetColor
            }, 0.015 * options.colorSpeedFactor * performance.factor * turboColorSpeedFactor * 300000)
        }
    ...
    function updateShape(shape) {
        ...
        for (var i = 0, len = shape.points.length; i < len; i++) {
            p1 = ChilLogo.util.getLoopedArrayElement(shape.points, i);
            ...
            if (ChilLogo.util.distanceBetween(p1, p1.normal) > MORPH_RANGE2) {
    ...
    function renderLogo() {
        ...
        renderShape(shapeBackground, new ChilLogo.util.Color(0, 0, 0, 0.5));
    ...
    function renderShape(shape, color) {
        ...
        var p1 = ChilLogo.util.getLoopedArrayElement(shape.points, -1);
        var p2 = ChilLogo.util.getLoopedArrayElement(shape.points, 0);
        logoContext.moveTo(p1.x + (p2.x - p1.x) / 2, p1.y + (p2.y - p1.y) / 2);
        for (var i = 0, len = shape.points.length; i <= len; i++) {
            p1 = ChilLogo.util.getLoopedArrayElement(shape.points, i);
            p2 = ChilLogo.util.getLoopedArrayElement(shape.points, i + 1);
